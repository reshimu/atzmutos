...
- [x] AT-11 | AGENT | risk:low | Delete 3 confirmed-dead legacy stubs: src/store.ts, src/intercept.ts, src/types/chayyot-verdict.ts — each is a one-line `export {}` with a comment naming its replacement; verified not imported  -> PR #5
...
- [x] AT-7 | AGENT | risk:low | Update atzmutos-build-pack-b.md risk-level list to include `BLOCKED` — NESHER emits SAFE/CAUTION/CRITICAL/CRITICAL/BLOCKED; spec only lists CRITICAL. The code is correct; the spec is stale. Fix the doc, not the code.  -> PR #4
